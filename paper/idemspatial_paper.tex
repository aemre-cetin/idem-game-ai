...
\title{\textbf{IdemSpatial: Deterministic $\mathcal{O}(N)$ In-Situ Spatial Partitioning and Volume-Preserving Swarm Dynamics via Idempotent Involutions with Zero Dynamic Allocation}}
\author{
    \textbf{Dr. A. Emre \c{C}ET\.{I}N} \\
    Computational Systems and Cognitive Architectures \\
    Izmir, Turkey \\
...
The systems, mathematical formulations, and algorithmic architectures disclosed herein are protected under \textbf{U.S. Patent Application No. 64/148,668} (Confirmation No. 5890, Priority Date: September 4, 2026). Commercial licensing inquiries should be addressed to Dr. A. Emre \c{C}ET\.{I}N.
